...
USERNAME: alice
PASSWORD: 5!fxoP3
These credentials are not known to the hacker, so they'd like to add a backdoor to this program, so that the username hacker and the password 1234 work. 
However, they wouldn't want to change the original file, so they're thinking of changing its "compiler", which takes this format and generates the following code:
